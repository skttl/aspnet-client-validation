/**
 * Options used in aspnet-client-validation
 */
...
/**
 * Class values to use for different elements in aspnet-client-validation
 */
...
export class MvcValidationProviders {
  ...
  compare: ValidationProvider = (value, element, params) => {
    ...
    let otherElement = getRelativeFormElement(
      element.name,
      params.other
    ) as HTMLInputElement;
    if (!otherElement) {
  ...
  remote: ValidationProvider = (value, element, params) => {
    ...
    for (let fieldSelector of fieldSelectors) {
      ...
      let fieldElement = getRelativeFormElement(
        element.name,
        fieldSelector
      ) as HTMLInputElement;
...
export class ValidationService {
  ...
  private elementEvents: {
    [id: string]: (e: Event, callback?: Function) => any;
  } = {};
  ...
  /**
   * Adds a validation message element generated by ASP.NET Core MVC to be managed by the service.
   * @param message
   */
  addMessage(message: HTMLElement) {
    let name = message.getAttribute("data-valmsg-for");

    if (!this.messageFor[name]) {
      this.messageFor[name] = [];
    }

    this.messageFor[name].push(message);
  }
  ...
  focusFirstInvalid = (form: HTMLFormElement) => {
    ...
    if (invalidFormInputUIDs.length > 0) {
      var firstInvalid = this.elementByUID[
        invalidFormInputUIDs[0]
      ] as HTMLElement;
      if (firstInvalid) {
        firstInvalid.focus();
      }
   ...
   * @param form The form to check validity on
   * @param prevalidate If validators should be run before checking validity
   * @param callback Callback function to run after running validators (if prevalidate is true)
   * @returns
   */
  isValid = (
   ...
   * @param field The field to check validity on
   * @param prevalidate If validators should be run before checking validity
   * @param callback Callback function to run after running validators (if prevalidate is true)
   * @returns
   */
  isFieldValid = (
    ...
    if (prevalidate) {
      if (field.form != null) {
        this.validateForm(field.form, callback);
      }
    }
  ...
  private trackFormInput(form: HTMLFormElement, inputUID: string) {
    ...
    form.addEventListener("reset", (e) => {
      ...
      for (let uid of uids) {
        ...
        input.classList.remove(...this.options.classes.validMessage.split(" "));
        input.classList.remove(
          ...this.options.classes.invalidMessage.split(" ")
        );
   ...
   */
  private renderSummary(form: HTMLFormElement) {
    let summaryElements = form.querySelectorAll('[data-valmsg-summary="true"]');
    ...
    for (let i = 0; i < summaryElements.length; i++) {
      ...
      e.innerHTML = "";
      if (ul) {
        e.className = this.options.classes.invalidSummary;
        e.appendChild(ul.cloneNode(true));
      } else {
        e.className = this.options.classes.validSummary;
      }
    }
  }
  ...
  addError(input: HTMLInputElement, message: string) {
    let spans = this.messageFor[this.getElementUID(input.form)][input.name];
    if (spans) {
      for (let i = 0; i < spans.length; i++) {
        spans[i].innerHTML = message;
        input.classList.remove(...this.options.classes.validMessage.split(" "));
        input.classList.add(...this.options.classes.invalidMessage.split(" "));
      }
    }

    input.classList.remove(...this.options.classes.validInput.split(" "));
    input.classList.add(...this.options.classes.invalidInput.split(" "));

    let uid = this.getElementUID(input);
    this.summary[uid] = message;
    this.renderSummary(input.form);
  }
  ...
  removeError(input: HTMLInputElement) {
    ...
    if (spans) {
      for (let i = 0; i < spans.length; i++) {
        ...
        input.classList.add(...this.options.classes.validMessage.split(" "));
      }
    }

    input.classList.remove(...this.options.classes.invalidInput.split(" "));
    input.classList.add(...this.options.classes.validInput.split(" "));
    ...
    return async () => {
      ...
      if (!this.isHidden(input)) {
        for (let key in directives) {
          ...
          if (!provider) {
            console.warn("aspnet-validation provider not implemented: " + key);
            continue;
          }
  ...
  private options: Options = {
    classes: {
      validInput: "input-validation-valid",
      validMessage: "field-validation-valid",
      validSummary: "validation-summary-valid",
      invalidInput: "input-validation-error",
      invalidMessage: "field-validation-error",
      invalidSummary: "validation-summary-errors",
    },
  };

  /**
   * Load default validation providers and scans the entire document when ready.
   * @param options Settings to use in the validation service
   */
